...
2. Тариф **Hacker** ($5/мес) — нужен ради своего домена, HTTPS и
   персистентного диска (на Free есть строгие лимиты трафика и только
   `*.pythonanywhere.com`).
...
Вкладка **Tasks** (доступна с тарифа Hacker) → добавить ежедневную задачу:
```
